...
# This function makes a new image that has the red values of the original image
# but sets the green and blue parts to zero. A pure green or blue color in the
# original will look black the new image since those colors get set to 0.
#
# Study the pattern of the loop inside a loop (called nested loops). Each time
# the first loop does a single repeat, the entire inner loop does all its
# repetitions.
#
# Look how to get the color at a pixel, change the color, and set the pixel to
# the new color.  The assignment problems below will use these steps.
...
def color_image_to_grey_scale(image):
    ...
    for y in range(0, image.getHeight()):
        for x in range(0, image.getWidth()):
            ...
            grey_image.setPixel(x, y, new_color)  # Set the pixel to the new color
...
def photonegative_of_an_image(image):
    negative = image.clone()  # copy the image
    # go across the image with the for x loop and then move down a row
    for y in range(0, image.getHeight()):
        for x in range(0, image.getWidth()):
            c = image.getPixel(x, y)  # get the pixel color
            new_color = [255 - c[0], 255 - c[1], 255 - c[2]]
            negative.setPixel(x, y, new_color)  # Set the pixel to the new color
    return negative  # return the finished new image
...
def brighten_image(image):
    bright = image.clone()  # copy the image
    # go across the image with the for x loop and then move down a row
    for y in range(0, image.getHeight()):
        for x in range(0, image.getWidth()):
            c = image.getPixel(x, y)  # get the pixel color
            new_color = [min(c[0] * 2, 255), min(c[1] * 2, 255), min(c[2] * 2, 255)]
            bright.setPixel(x, y, new_color)  # Set the pixel to the new color
    return bright  # return the finished new image
...
# Ask yourself, if you want to repeat the action 50000 times, what kind of loop
